...
$stmt=$mysqli->prepare('SELECT occupied FROM courses WHERE no=? AND occupied<total');
...
if($stmt->fetch()){
    ...
    $stmt=$mysqli->prepare('UPDATE courses SET occupied=? WHERE no=?');
    $occupied++;
    ...
    if($chosen!=-1){
        ...
        $stmt=$mysqli->prepare('SELECT occupied FROM courses WHERE no=?');
    ...
    $stmt=$mysqli->prepare('UPDATE user_info SET enrollment=? WHERE username=?');
