...
if(!check_privilege(PRI_ACCESS_PROJECTS)){
    echo 'ungranted';
    exit();
}
$mysqli=getDB();
...
$stmt->close();
$query_string='SELECT no,name,sponsor,occupied,total FROM projects WHERE name LIKE ? AND sponsor LIKE ? ';
if(get_para('av')=='true')
...
while($stmt->fetch()){
    ...
    }elseif(!check_privilege(PRI_ENROLL_PROJECT)){
